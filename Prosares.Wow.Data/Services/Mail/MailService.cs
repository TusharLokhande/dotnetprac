...
namespace Prosares.Wow.Data.Services.Mail
{
    public class MailService:IMailService
    {
        ...
        private readonly IRepository<ConfigTable> _configTable;

        private readonly ILogger<MailService> _logger;


        public MailService(IConfiguration config, IRepository<ConfigTable> configTable, ILogger<MailService> logger)
        {
            _config = config;
        ...
        public bool SendMail(SendMailModel MailDetails)
        {
            ...
            _logger.LogInformation("Mail Service Started ", "Started", DateTime.UtcNow);

            bool returnValue;
            MailMessage message = new MailMessage();
         

            var SmtpConfig = _configTable.GetAll(b=>b.Where(k => k.ConfigKey == "Smtp")).FirstOrDefault();

            if (SmtpConfig != null)
            {
                SMTPModel smtpDetails = JsonConvert.DeserializeObject<SMTPModel>(SmtpConfig.ConfigValue);
            ...
            try
            {
                ...
                string FromName = !String.IsNullOrEmpty(MailDetails.FromName) ? MailDetails.FromName : emailAccountEntity.DisplayName;
                string FromAddress = !String.IsNullOrEmpty(MailDetails.From) ? MailDetails.From : emailAccountEntity.Email;

                // if (!String.IsNullOrEmpty(MailDetails.FromName) && MailDetails.FromName != "")
                // {
                ...
                _logger.LogInformation("Mail Service Ended ", "Ended", DateTime.UtcNow);
